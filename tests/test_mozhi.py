...
def cdbfile(tmp_path_factory):
	jafile = str(tmp_path_factory.mktemp('mozhi') / (Path(__file__).stem + '.ja'))
...
def test_plurals(cdbfile):
	...
	assert ja.p('{} month', '{} months', 1) == '1 month'
	assert ja.p('{} month', '{} months', 2) == '2 months'
	assert ja.p('{} month', '{} months', 3) == '3 months'
	assert ja.p('{} month', '{} months', 11) == '11 months'
	assert ja.p('{} month', '{} months', 20) == '20 months'
	ja.set_langs('pl')
